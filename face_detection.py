    ...
    def face_detect(self, frame):
        ...
        if len(rects)>0:
            ...
            if len(rectsf) >0:
                ...
                BLK = np.zeros(face_frame.shape, np.uint8)
                cv2.rectangle(BLK,(shape[54][0], shape[29][1]), #draw rectangle on right and left cheeks
                        (shape[12][0],shape[33][1]), (0,255,0), -1)
                face_frame = cv2.addWeighted(face_frame, 1.0, BLK, 0.25, 1)
                BLK = np.zeros(face_frame.shape, np.uint8)
                cv2.rectangle(BLK, (shape[4][0], shape[29][1]),
                        (shape[48][0],shape[33][1]), (0,255,0), -1)
                face_frame = cv2.addWeighted(face_frame, 1.0, BLK, 0.25, 1)
            ...
            cv2.putText(frame, "No face detected",
                       (150,200), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0, 0, 255),4)
            status = False
        return frame, face_frame, ROI1, ROI2, status, mask
