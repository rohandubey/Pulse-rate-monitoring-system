...
class Camera():
    def __init__(self):
        if opt.cam == None:
            self.cap = cv2.VideoCapture(opt.source)  # Prepare the camera...
        else :
            self.cap = cv2.VideoCapture(opt.cam)  # Prepare the camera...
...
if __name__ == '__main__':
    ...
    parser.add_argument('--source', type=str, default='inference/images', help='video source')  # file/folder, 0 for webcam
    parser.add_argument('--cam', type=int, default=None, help='webcam source')
    parser.add_argument('--move', type=bool, default=False, help='moving source/camera')
